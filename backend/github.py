    ...
    if rate_limit_remaining is not None and rate_limit_limit is not None:
        ...
        if remaining < 10 and rate_limit_reset:
            ...
            if wait_seconds > max_wait:
                print(
                    f" Rate limit reset time is too far in the future ({wait_seconds}s). Capping wait to {max_wait}s"
                )
            ...
            logger.info(
                f"ℹ  GitHub API rate limit: {remaining}/{limit} requests remaining"
            )
    ...
    try:
        ...
        try:
            ...
            if len(unique_projects) < 7:
                print(
                    f"LLM selected {len(selected_projects)} projects but {len(unique_projects)} are unique"
                )
